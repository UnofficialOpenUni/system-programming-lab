...
#define N 5
#define MAX_MEMBERS ((N)*(N))								/* Max amount of members in an NxN sized square */
#define CONSTANT_SUM ((N) * (MAX_MEMBERS + 1)/2)			/* The formula for a magic square's constant sum */
#define IsConstSum(SUM) ((SUM) == CONSTANT_SUM)				/* validates if a sum is a valid magic sum */
#define ValidMagicMember(X) ((X) > 0 && (X) <= MAX_MEMBERS)	/* validates if a value is a valid magic member */
#define FAILURE -1
...
enum _BitMagicFlags {
	VALID = 0x0u, 						/* 0 */
	INVALID_MAGIC_MEMBER = 0x1u,		/* 1 */		/* Describes values used for */
	MULTI_MEMBER_APPEARANCE = 0x2u,		/* 2 */		/* bit operations, representing*/
...
void setMemTable(Square square, MagicTable tab);
...
void addToTable(MagicTable tab, int val);
